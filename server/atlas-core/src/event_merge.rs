...
pub const EVENT_MERGE_PAIRS: &[EventMerge] = &[
    ...
    EventMerge { survivor: "ab_egypt", absorbed: "theo-73", reason: "Batch CHRON-1 (THE CHRONOLOGY AUTHORITY LAW, DUPLICATE_JACCARD_THRESHOLD lowered to 0.5): theo-73 \"Abraham goes to Egypt\" (jaccard 0.727 AT FLAG TIME, Gen.12.10-20) and ab_egypt \"Famine drives Abram to Egypt\" (curated) narrate the identical occasion. ab_egypt's own pre-fix citation (GEN.12.10+GEN.12.14-20) deliberately skipped GEN.12.11-13 (Abram's instruction to Sarai to pose as his sister); FIX ROUND 1 (review finding I-1): that gap is CLOSED, not merely disclosed -- ab_egypt's own top-level `verses` is now the single contiguous GEN.12.10-20 (data/curated/events-extra.toml), restoring theo-73's own boundary coverage; no separate curated event claims 12:11-13 either way. ab_egypt survives; theo-73 absorbed." },
    ...
    EventMerge { survivor: "je_egypt_ruler", absorbed: "theo-111", reason: "Batch CHRON-1 (THE CHRONOLOGY AUTHORITY LAW, DUPLICATE_JACCARD_THRESHOLD lowered to 0.5): theo-111 \"Joseph Promoted in Egypt\" (jaccard 0.600 AT FLAG TIME, Gen.41.37-46) and je_egypt_ruler \"Joseph rises to rule over Egypt\" (curated) narrate the identical occasion -- Pharaoh's elevation of Joseph -- theo-111's own citation opened 4 verses earlier (41:37-40, Pharaoh's own deliberation before the appointment) than je_egypt_ruler's own pre-fix GEN.41.41-46. FIX ROUND 1 (review finding I-2): je_egypt_ruler's own top-level `verses` is now GEN.41.37-46 (data/curated/events-extra.toml) -- the survivor itself claims Gen.41.37-40 now, closing what this reason used to call an unclaimed gap. je_egypt_ruler survives; theo-111 absorbed." },
    ...
    EventMerge { survivor: "pw_temple_cleansing", absorbed: "theo-445", reason: "Batch CHRON-1 (THE CHRONOLOGY AUTHORITY LAW, DUPLICATE_JACCARD_THRESHOLD lowered to 0.5): theo-445 \"Temple Cleansed\" (jaccard 0.667, Matt.21.12-17+Mark.11.15-19+Luke.19.45-48) and pw_temple_cleansing \"Jesus cleanses the temple a second time\" (curated, Robertson Section 129, MAT.21.12-13 top-level + MRK.11.15-18/LUK.19.45-48 witnesses) narrate the same cleansing; theo-445's own small extra tail (MAT.21.14-17 healing-in-the-temple + children's Hosanna, MRK.11.19) is not preserved onto the survivor (container-algebra: survivor's own fields stand unmodified) -- FIX ROUND 1 (review finding T-3): this is NOT a coverage gap, corrected from the original reason's own overcautious claim -- `theo-443` (the curated Holy Week container) already covers all five of those verses in the shipped export, verified directly. pw_temple_cleansing survives; theo-445 absorbed." },
    ...
    EventMerge { survivor: "rob_leper_healed", absorbed: "theo-286", reason: "THE CHARTER CASE (Batch CHRON-1, owner ruling: \"why are we pulling chronology from conflicting sources? we should have one absolute source of truth\"): theo-286 \"Healing the Leper\" (jaccard 0.733 AT FLAG TIME, Matt.8.1-4+Mark.1.40-44+Luke.5.12-14, Theographic-calibrated AD 31) and rob_leper_healed \"A leper healed; great popular excitement\" (curated, Robertson (1922) Section 45, AD 30, MRK.1.40-45/LUK.5.12-16 witness rows) are the identical Gospel pericope -- the exact defect the whole batch exists to fix, invisible to the former 0.8 sweep floor (leper-pair jaccard 0.733) and to the cross-book title detector (title jaccard only ~0.167). rob_leper_healed survives (the richer, Robertson-grounded, witness-complete curated container, per THE CHRONOLOGY AUTHORITY LAW's own precedence order); theo-286 absorbed. FIX ROUND 1 (review finding S-C1/S-1): rob_leper_healed's own top-level `verses` was widened MAT.8.2-4 -> MAT.8.1-4 to absorb theo-286's own MAT.8.1 boundary verse, but `scene::witnesses_for` returns ONLY explicit witness rows once any exist (rob_leper_healed already carried MRK/LUK/MAT ones) -- the top-level widening alone never reached a real reader; the MAT witness row itself (data/curated/event-witnesses.toml) is now also MAT.8.1-4, the fix that actually restores the compiled coverage. MAT.8.3 (and MAT.8.1) now surface exactly one event. BATCH ATTEST-1 (owner order 1, verbatim: \"A leper healed; a great popular excitement is given a parallel where there shouldn't be from Mat.8.1-4; another leprosy story\"): THE MERGE ITSELF STANDS -- theo-286 is still absorbed into rob_leper_healed -- but the fix-round widening described immediately above was re-homed. theo-286 fused Matthew's leper with Mark's and Luke's (Theographic's own harmonization), and absorbing it carried that fusion onto the survivor as a MAT witness row, which is the false parallel the owner reported: Matthew 8:1 dates its own occasion \"when he was come down from the mountain\" (after the Sermon on the Mount), while Mark 1:40-45 and Luke 5:12-16 place theirs during the first Galilean tour and close with the publishing-abroad aftermath rob_leper_healed is titled for. MAT.8.1-4 now attests the NEW mat_leper_healed event (data/curated/events-extra.toml) and the two events are joined by an Analogue row (data/curated/attestation-corrections.toml) -- ZERO coverage lost, the same four verses still attested, by the event whose occasion they actually narrate. ROOT CAUSE, recorded here because this table is where it entered: a verse-set similarity metric cannot tell \"one event narrated in three Gospels\" from \"two similar events in different Gospels\"; only the text can, and Analogue is the vocabulary that lets the answer be written down." },
    ...
    EventMerge { survivor: "rob_twelve_apostles", absorbed: "theo-295", reason: "Batch CHRON-1 fix round 1 (review finding I-3, re-triaged from EVENT_DISTINCT_PAIRS -- reversed on the merits, not merely on instruction): theo-295 \"Apostles Chosen\" (jaccard 0.733, Matt.10.2-4+Mark.3.13-18+Luke.6.12-16) and rob_twelve_apostles \"Jesus selects the twelve apostles after a night of prayer\" (curated, Robertson Section 53, MRK.3.13-19 top-level + LUK.6.12-16 witness) share the IDENTICAL 0.733 shape this batch already merged twice over in Acts (p1_pisidian_antioch/theo-340, extra verse owned by p1_perga; p1_lystra/theo-342, extra verses owned by p1_iconium): theo-295's only disjoint material, MAT.10.2-4, is ALREADY claimed by the separately-curated rob_third_tour_sending_twelve (this entry's own former reason correctly named that fact) -- but per this module's own container-algebra law, merging NEVER writes the absorbed side's own verses onto the survivor, so collapsing theo-295 into rob_twelve_apostles asserts nothing about when Matthew's later apostle-list moment happened; the harmony-timing objection that justified DISTINCT does not actually bite the identity question. The compiled placements already agreed (both AD 31, order_key/seq the only difference), so the pre-fix-round shape shipped a duplicate CARD at MRK.3.13-18/LUK.6.12-16, not a contradictory date -- but a duplicate card is still exactly what THE CHRONOLOGY AUTHORITY LAW forbids. rob_twelve_apostles survives (curated, Robertson-grounded); theo-295 absorbed." },
    ...
    EventMerge { survivor: "theo-152", absorbed: "theo-145", reason: "Batch CHRON-1 (THE CHRONOLOGY AUTHORITY LAW, DUPLICATE_JACCARD_THRESHOLD lowered to 0.5), layer0-layer0, no curated counterpart for either side: theo-152 \"Death of Eli, Ark Captured\" (jaccard 1.000, 1Sam.4.15-18) cites EXACTLY the same four verses as theo-145 \"Judgeship of Eli\" (1Sam.4.15-18) -- a full verse-set identity, not merely an overlap, so unlike the genuine \"Lifetime of X\"/\"Birth-or-Death of X\" mega-span pattern elsewhere in this table (see the Genesis genealogy DISTINCT entries below), there is no disjoint material on either side to justify keeping two ids: this is Theographic's own citation quirk (its \"Judgeship of Eli\" event cites only the closing Ark-capture scene rather than a proper 1Sa.1-4 span), not two accounts of two different moments. FIX ROUND 1 (review finding I-4): the original entry survived theo-145 on \"the more informative title\" reasoning alone, without checking WHERE that title's own date compiles to -- theographic events compile to a POINT (`startDate` only, never `duration`-extended, `theographic.rs`'s own parse), so surviving on theo-145's own `startDate=-1161` (the 40-year judgeship's own START) placed these four verses -- Eli's own death, the Ark's own capture -- roughly 40 years before they happened, under a title (\"Judgeship of Eli\") that names NEITHER the death nor the capture. theo-152 survives instead: its own `startDate=-1121` is the real, accurately-dated year these four verses actually narrate, and its own title names the scene directly. theo-145 absorbed -- the judgeship's own 40-year span itself is not separately preserved (container-algebra: no curated event claims 1Sa.1-4's earlier material either way, before or after this switch; a real gap in the OT judge backbone, disclosed, not this fix round's own charter to close)." },
