...
// other lines in the narrative"). CONTRACT.md's own NARRATIVE-1 note.
//
...
test('NARRATIVE-1: map focus classes flip on popover open, follow traversal, and clear on close', async ({ page }) => {
  ...
  // reader, split view, and map-side popovers."
...
test('NARRATIVE-1: data-narrative-focus state changes carry no CSS transition (reduced-motion is a no-op change, by design)', async ({ page }) => {
  await page.goto(`/world?from=${EXODUS_WINDOW.from}&to=${EXODUS_WINDOW.to}`);
