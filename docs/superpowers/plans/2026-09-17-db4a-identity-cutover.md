...
3. **ORDER-1 lands first, alone, as Task 1.** Every chronology sort gets the event id as its final key so the order is total and SQL can `ORDER BY (from_year, order_key, id)`. The graph-served path (`scene_source.rs`) already sorts id-ordered input stably by `from_year`, so its order is unchanged by construction; the compile-time chronology (`event_world::timeline_order`) tie-breaks by array position today and moves to id order — that moves `temporal_adjacency` rows, `chronology.order`, `/api/narrative/{id}/event-positions` prior/following inside same-`(from_year, order_key)` groups (137 groups in the real data), the exports' row order, and the root. Task 1 measures whether the 25 scene hashes move and re-pins them only if they do, with the count of moved windows in the commit message.
