...
- **map-generator needs nothing from this batch** (owner, 2026-09-17: "stop worrying about planning the map generator ingestion"). The survey found it imports none of the touched types (a live path dependency, no vendored copy, no exhaustive match on any atlas enum) and its `atlas-edge` suite is pure HTTP over six endpoints whose JSON this batch does not touch. It is never edited and not checked here.
