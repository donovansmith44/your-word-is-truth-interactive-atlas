...
| Esri `World_Shaded_Relief` tile service (Batch S correction: this row named the earlier, since-replaced `NatGeo_World_Map` pick — Batch C swapped the live basemap to `World_Shaded_Relief`, see `client/wwwroot/js/map.js`'s own `TILE_URL`; this row had gone stale and is corrected here, matching the app's own Credits disclosure, which already said "World Shaded Relief") | Esri terms of use | **Never redistributed by this repo.** Tiles are fetched live by the browser at runtime from Esri's own servers, with the attribution string carried verbatim in the app UI and in `client/wwwroot/js/map.js`'s `TILE_ATTRIBUTION`. |
...
| Catechism reflection questions (`data/curated/catechism-reflection.toml`, CATECH-V1) | **CC0 1.0 Universal** (public domain dedication) | Ours. 165 hand-authored teaching prompts across all 33 items, tiered child/youth/adult and ordered easy-to-difficult; compiled into `catechism.json` (`CatechismItem.reflection`). Grounded in each item's own Luther text (which is public domain, above) but the QUESTIONS themselves are original to this project — they are prompts a teacher asks, not quotations of any source. |
| Catechism song bindings (`data/curated/catechism-music.toml`, CATECH-V1) | **CC0 1.0 Universal** as to the CURATION (which hymn pairs with which item) | Ours — but note carefully what is and is not redistributed: this file contains only LINKS plus our own note on why each pairing fits. **No audio, no recording, and no hymn text is redistributed by this repo.** A `spotify`/`youtube` entry is handed to that provider's own embed player at runtime (their terms govern playback, exactly like the Esri/Carto tiles above); an `mp3` entry is a URL the browser plays directly, and if it points at a file you host yourself, that file's licensing is yours to settle, not this repo's. |
