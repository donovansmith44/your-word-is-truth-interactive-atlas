...
pub const EVENT_MERGE_PAIRS: &[EventMerge] = &[
    ...
    EventMerge { survivor: "jm_sychar", absorbed: "theo-278", reason: "Batch CHRON-1 (THE CHRONOLOGY AUTHORITY LAW, DUPLICATE_JACCARD_THRESHOLD lowered to 0.5): theo-278 \"The Samaritan Woman\" (jaccard 0.564, John.4.5-42) and jm_sychar \"Jesus speaks with the woman at the well in Sychar\" (curated, Robertson Section 35, JHN.4.4-26) are the same episode; theo-278's own trailing verses (JHN.4.27-42, the disciples' return and the Samaritans' own belief) have no separate curated container of their own -- an honest, disclosed gap (unlike jm_jordan/theo-267's own precedent, no witness-row enrichment is authored here; container-algebra keeps the survivor's own fields unmodified). jm_sychar survives; theo-278 absorbed." },
