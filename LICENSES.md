...
(the `concord` section under `data/compiled/sections/`; DB-5 retired
...
| **DB-5 (2026-09-18):** every `data/compiled/*.json` row below (except `sources.json`) now describes a TABLE of the `core` section under `data/compiled/sections/` -- `canon.json` -> `canon_book`/`canon_chapter_verses`, `books-meta.json` -> `book_meta`, `chronology-anchors.json` -> `chronology_anchor`, `book-narration-windows.json` -> `book_narration_window`, `landmarks.json` -> `landmark`, `land-mask.json` -> `land_mask_region`, `catechism.json` -> `catechism_*`, `place-history.json` -> `place_history*`, `place-names-kjv.json` -> `place_name_alias*`; `polities.json` is retired (the `Polity` node payloads are the source). The license dispositions are unchanged; the files are not. | | |
